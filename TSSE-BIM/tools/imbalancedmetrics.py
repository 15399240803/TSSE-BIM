...
class ImBinaryMetric:
    ...
    def gmean(self):
        ...
        # return np.sqrt(((1.0*self.TN)/N)*((1.0*self.TP)/P))*100
        return geometric_mean_score(self.y_true,self.y_pred)*100
    def recall(self):
        return recall_score(self.y_true,self.y_pred)*100
    
    def precision(self):
        return precision_score(self.y_true,self.y_pred)*100
    
    def f1(self):
        return f1_score(self.y_true,self.y_pred)*100
    ...
    '''
    def MCC(self):
        return matthews_corrcoef(self.y_true,self.y_pred)*100
    def AP(self):
        return average_precision_score(self.y_true,self.y_pred)*100
    
    def aucroc(self,y_preprob):
        y=self.y_true.copy()
        auc=roc_auc_score(y,np.array(y_preprob))
        prc = average_precision_score(y,np.array(y_preprob))

        # auc = roc_auc_score(y_test,clf.decision_function(X_test))
        # fpr, tpr, thresholds = roc_curve(y, np.array(y_preprob))
        # print(fpr)
        # print(tpr)
        # print(thresholds)
        # print(auc)
