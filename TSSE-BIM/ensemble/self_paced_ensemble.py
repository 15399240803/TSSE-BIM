...
class SelfPacedEnsemble(BaseEstimator, ClassifierMixin):
    ...
    def self_paced_under_sampling(self, 
        ...
        # Else allocate majority samples into k hardness bins
            ...
            # wights=[]
            # for i in range(0, len(ave_contributions)):
            #     if ave_contributions[i] < 0.25 :
            #         wights.append(0)
            #     elif ave_contributions[i] >= 0.25 and ave_contributions[i] < 0.35:
            #         wights.append(0)
            #     elif ave_contributions[i] >= 0.35 and ave_contributions[i] < 0.7:
            #         wights.append(0.2)
            ...
            plt.scatter(X_train_maj[:, 0], X_train_maj[:, 1], c='b', marker='*')
            plt.scatter(X_min[:, 0], X_min[:, 1], c='r', marker='*')
            plt.show()
