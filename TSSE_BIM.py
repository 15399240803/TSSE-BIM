...
class TSSE(BaseEstimator, ClassifierMixin):
        ...
        if flag ==0:
            ...
            for i in range(0,len(noempty_bin_key)):
                ...
                if ave_contributions[i] < 0.2:
                    wights.append(0)
                    #plt.scatter(X_maj[part_bin==noempty_bin_key[i],0], X_maj[part_bin==noempty_bin_key[i],1], c='skyblue',marker='_')
                elif ave_contributions[i] >= 0.2 and ave_contributions[i] < 0.4:
                    wights.append(0.5)
                elif ave_contributions[i] >= 0.4 and ave_contributions[i] < 0.6:
                    wights.append(1)
                    #plt.scatter(X_maj[part_bin == noempty_bin_key[i], 0], X_maj[part_bin == noempty_bin_key[i], 1],c='deepskyblue', marker='+')
                elif ave_contributions[i] >= 0.6 and ave_contributions[i] < 0.7:
                    wights.append(5)
                    #plt.scatter(X_maj[part_bin == noempty_bin_key[i], 0], X_maj[part_bin == noempty_bin_key[i], 1],c='royalblue', marker='s')
                elif ave_contributions[i] >= 0.7 and ave_contributions[i] < 0.95:
                    wights.append(5)
                    #plt.scatter(X_maj[part_bin == noempty_bin_key[i], 0], X_maj[part_bin == noempty_bin_key[i], 1],c='blue',marker='|')
                elif ave_contributions[i] >= 0.95 :
                    wights.append(2)
                    #plt.scatter(X_maj[part_bin == noempty_bin_key[i], 0], X_maj[part_bin == noempty_bin_key[i], 1],c='navy',marker='X')

            for i in range(0, len(noempty_bin_key)):
                # s[noempty_bin_key[i]] = self.k_bins*wights[i]
                # s[noempty_bin_key[i+1]] += s[noempty_bin_key[i]]*(1-wights[i])
                s[noempty_bin_key[i]] *= wights[i]
                # s[noempty_bin_key[i]]=min(len(prob[part_bin==noempty_bin_key[i]])*wights[i],s[noempty_bin_key[i]])
                # s[noempty_bin_key[i]] = (len(prob[part_bin == noempty_bin_key[i]]))*wights[i]
            # for i in range(0, len(noempty_bin_key)):
            #     if s[noempty_bin_key[i]]!=0:
            #         s[noempty_bin_key[i]]=0
            #         break
            # s[noempty_bin_key[i+1]] = 0
            # print(ave_contributions)
            # s[noempty_bin_key[-1]] = len(prob[part_bin == noempty_bin_key[-1]])
    ...
    def fit(self, X, y, label_maj=0, label_min=1):
        ...
        # self.s_bins = math.ceil(X_min.shape[0] * (1 - l_x_min / len(X_min)+1))
